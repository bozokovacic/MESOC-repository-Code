...
<!--            {{ link_to("documentlink/linkslitarea/" ~ document.ID_Doc, 'Literature Area', "class": "btn btn-primary") }} -->
            ...
            {{ link_to("documentlink/linkscity/" ~ document.ID_Doc, 'City', "class": "btn btn-primary") }} 

 <h3>CITY</h3> 

{% for doccity in page.items %}
    {% if loop.first %}
<table class="table table-bordered table-striped" align="center">
    <thead>
        <tr>
    ...
    {% endif %}
        <tr>
        <td>{{ doccity.getCity().CityName }}</td>       
        <td with="7%">{{ link_to("documentlink/citydelete/" ~ doccity.ID_City, '<i class="glyphicon glyphicon-remove"></i> Delete city', "class": "btn btn-default") }}</td>
...
{{ form("documentlink/citysearch/" ~ document.ID_Doc) }}
