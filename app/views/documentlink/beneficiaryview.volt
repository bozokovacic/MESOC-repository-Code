...
{% for docbeneficiary in page.items %}
    {% if loop.first %}
<table class="table table-bordered table-striped" align="center">
    <thead>
        <tr>
    ...
    {% endif %}
        <tr>
        <td>{{ docbeneficiary.ID_Beneficiary }}</td>
        <td>{{ docbeneficiary.getBeneficiary().BeneficiaryName }}</td>
        <td with="7%">{{ link_to("documentlink/beneficiarydelete/" ~ docbeneficiary.ID_Beneficiary, '<i class="glyphicon glyphicon-remove"></i> Delete', "class": "btn btn-default") }}</td>
...
{{ form("documentlink/beneficiarysearch/" ~ document.ID_Doc) }}

<h3>ADD BENEFICIARY TO DOCUMENT</h3>
