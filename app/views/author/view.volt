    ...
    <tbody>
        <tr>
            ...
            <td>{{ author.MiddleNameInitial }}</td>
                <td width="7%">{{ link_to("author/edit/" ~ author.ID_Author, '<i class="glyphicon glyphicon-edit"></i> Edit', "class": "btn btn-default") }}</td>
            <td width="7%">{{ link_to("author/delete/" ~ author.ID_Author, '<i class="glyphicon glyphicon-remove"></i> Delete', "class": "btn btn-default") }}</td>
        </tr>
    </tbody>
{% if loop.last %}
    <tbody>
