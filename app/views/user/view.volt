...
{% endif %}
    <tbody>
        <tr>
            <td>{{ user.ID_User }}</td>
            <td>{{ user.UserName }}</td>
                <td width="7%">{{ link_to("user/edit/" ~ user.ID_User, '<i class="glyphicon glyphicon-edit"></i> Edit', "class": "btn btn-default") }}</td>
            <td width="7%">{{ link_to("user/delete/" ~ user.ID_User, '<i class="glyphicon glyphicon-remove"></i> Delete', "class": "btn btn-default") }}</td>
        </tr>
    </tbody>
{% if loop.last %}
    <tbody>
