...
{% endif %}
    <tbody>
        <tr>
            ...
            <td>{{ role.RoleName }}</td>
            <td width="7%">{{ link_to("role/edit/" ~ role.ID_Role, '<i class="glyphicon glyphicon-edit"></i> Edit', "class": "btn btn-default") }}</td>
            <td width="7%">{{ link_to("role/delete/" ~ role.ID_Role, '<i class="glyphicon glyphicon-remove"></i> Delete', "class": "btn btn-default") }}</td
         </tr>
    </tbody>
{% if loop.last %}
    <tbody>
